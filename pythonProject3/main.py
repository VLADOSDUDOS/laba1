...
# Создаем собственное исключение для старых фильмов
class OldMovieException(Exception):
    def __init__(self, year):
        self.year = year
        super().__init__(f"Old Movie Released in {year}")
...
try:
    ...
    movie = Movie("Movie 1", 2000, "Director 1", action_genre, 120, [actor1, actor2])
    tv_show = TVShow("TV Show 1", 2023, "Director 2", 5, 10)

    # Проверяем, если фильм старше 2000 года, вызываем исключение
    if movie.year < 2000:
        raise OldMovieException(movie.year)
